                ...
                if ( thisList is not null )
                {
                    for ( int i = 0; i < thisLog.TagNames.Count; i++ )
                    {
                        ...
                        foreach ( CustomLogValue entry in thisList )
                        {
                            try
                            {
                                if ( entry.Value[ i ] is not null ) sb.Append( $"[{CuSupport.DateTimeToJSUTC( entry.Date )},{entry.Value[ i ]?.ToString( "F2", CUtils.Inv )}]," );
                                else sb.Append( $"[{CuSupport.DateTimeToJSUTC( entry.Date )}, null]," );
                            }
                            catch { continue; }
                        }
